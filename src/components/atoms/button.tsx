...
const buttonVariants = cva(
  'relative inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-[15px] text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'bg-primary text-primary-foreground hover:bg-primary/90 font-[600]',
        destructive:
  ...
  (
    ...
    return (
      ...
      >
        {loading ? (
          <>
            <div className='w-15 absolute inset-0 flex items-center justify-center'>
  ...
  ({ className, asChild = false, active = false, ...props }, ref) => {
    return (
      <Button
        ...
        {...props}
        className={cn(
          'h-[20px] text-[16px] text-text-base-primary no-underline hover:no-underline',
          active ? 'cursor-auto font-[700]' : 'font-[400]',
          className,
        )}
      />
    );
...
export { Button, buttonVariants, TabButton };
